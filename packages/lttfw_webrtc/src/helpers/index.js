...
export function getCurrentLang() {
    ...
    return lang ? lang : 'zh_hk'
}

export function getCurrentThemeName() {
    const themeName = localStorage.getItem('themeName')
    return themeName ? themeName : 'classic_light'
}
...
export function switchStream(stream, peer) {
    Object.values(peer?.connections).forEach((connection) => {
        ...
        const senders = connection[0].peerConnection.getSenders()
...
export function onStreamChanged(store, slices, currentPeer, currentStream) {
...
export function isEmptyConstraints(constraints) {
    return _.isEmpty(constraints) || (!constraints.video && !constraints.audio)
}

export function devicesGrouped(devices) {
    const video = []
    const audio = []
    devices.map((device)=>{
        if (device.kind === 'videoinput') {
            if (_.find(video, (v)=>v.deviceId == device.deviceId)) {
                return
            }
            video.push(device)
        }
        if (device.kind === 'audioinput') {
            if (_.find(audio, (v)=>v.deviceId == device.deviceId)) {
                return
            }
            audio.push(device)
        }
...
export function getDevices(params) {
    ...
    if (!navigator.mediaDevices?.enumerateDevices) {
        ...
        .then((devices) => {
            ...
            success && success(grouped)
        })
        .catch((err) => {
          error ? error(err) : console.error(`${err.name}: ${err.message}`)
        })
    }
}

export function addEvent(store, slices, severity, type, title, message) {
